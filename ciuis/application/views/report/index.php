				...
				<div class="panel rad-5 panel-default printerareaciuis">
					...
					<div class="tab-container">
						<ul class="nav nav-tabs">
							<li class="active"><a href="#financial_summary" data-toggle="tab"><?php echo lang('financialsummary');?></a></li>
							<li><a href="#invoice_ist" data-toggle="tab"><?php echo lang('invoicessummary');?></a></li>
							<li><a href="#customer_ist" data-toggle="tab"><?php echo lang('customersummary');?></a>	</li>
							<li><a href="#leads_ist" data-toggle="tab">Leads Summary</a>	</li>
						</ul>
						<div class="tab-content">
							...
							<div id="leads_ist" class="tab-pane">
								<div class="row">
									<div class="col-md-12 animated fadeIn">
										<div class="panel">
											<div class="panel-heading">
											...
											<div class="panel-body">
												<div class="row">
													<div class="col-md-3">
														<?php
														echo '<select name="lm" class="form-control select2" data-none-selected-text="April">' . PHP_EOL;
														for ( $lm = 1; $lm <= 12; $lm++ ) {
															$_selected = '';
															if ( $m == date( 'lm' ) ) {
																$_selected = ' selected';
															}
															echo '  <option value="' . $lm . '"' . $_selected . '>' . ( date( 'F', mktime( 0, 0, 0, $lm, 1 ) ) ) . '</option>' . PHP_EOL;
														}
														echo '</select>' . PHP_EOL;
														?>
													</div>
										...
										<div class="col-md-12" style="background: #febb00; border-radius: 10px;padding: 10px">
...
<script>
	new Chart($('#leads_to_win_by_leadsource'), {
             type: 'horizontalBar',
             data: <?php echo $leads_to_win_by_leadsource; ?>,
			 options: {
				 legend: {
					display: false,
				 }
...
<script>
	...
	var lead_graph;
	$( function () {
		$.get( ciuis_url + 'report/lead_graph/' + $( 'select[name="lm"]' ).val(), function ( response ) {
		...
		$( 'select[name="lm"]' ).on( 'change', function () {
			lead_graph.destroy();
			$.get( ciuis_url + 'report/lead_graph/' + $( 'select[name="lm"]' ).val(), function ( response ) {
				var ctx = $( '#lead_graph' ).get( 0 ).getContext( '2d' );
				lead_graph = new Chart( ctx, {
