...
<div class="ciuis-body-content">
	<div class="main-content container-fluid col-xs-12 col-md-12 col-lg-12">
 	<div class="col-md-12 borderten md-p-0">
	  ...
	  <?php foreach($leadsstatuses as $status){?>
	  <?php $leads = $this->db->select( '*,leadsstatus.name as statusname,staff.staffname as leadassigned,staff.staffavatar as assignedavatar,leadssources.name as sourcename,leads.name as leadname,leads.id as id' )->order_by('leads.id', 'asc')->join( 'leadsstatus', 'leads.status = leadsstatus.id', 'left' )->join( 'leadssources', 'leads.source = leadssources.id', 'left' )->join( 'staff', 'leads.assigned = staff.id', 'left' )->where( 'public != "0" AND  staff = '.$this->session->userdata( 'logged_in_staff_id' ).' AND assigned = '. $this->session->userdata( 'logged_in_staff_id' ) .' OR status = "'.$status['id'].'"' )->get( 'leads')->result_array(); ?>
		  ...
		  <?php foreach($leads as $lead){?>
			<li id="<?php echo $lead['id']?>" class="leadcard">
			  <dl  data-lead="<?php echo $lead['id']?>">
				...
				<dd class="date">Added: <?php echo _adate($lead['created']) ?></dd>
			  </dl>
			</li>
		  <?php }?>	
		  </ul>
