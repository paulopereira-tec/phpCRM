				...
				<ul>
					<li ng-repeat="invoice in overdueinvoices" class="next overdueinvoices">
						...
						<p>
							...
							<span class='location text-bold'><span class="money-area" ng-bind="invoice.total"></span></span><br>
					...
					<li ng-repeat="invoice in dueinvoices" class="next dueinvoices">
						...
						<p>
							...
							<span style="color:black;" class='location text-bold'><span class="money-area" ng-bind="invoice.total"></span></span>
						</p>
					</li>
					<li ng-repeat="transaction in transactions" class="next {{transaction.type}}">
						...
						<p><span style="color:black;" class='duration text-bold'><span class="money-area" ng-bind="transaction.amount"></span></span>
						</p>
					</li>
					<li ng-repeat="ticket in newtickets" class="next newticketsidebar">
						...
						<h3 class="text-bold" ng-bind="ticket.subject"></h3>
						<p>
							<span style="color:black;" class='duration text-bold' ng-bind="ticket.contactname +' '+ ticket.contactsurname"></span>
							<span style="color:black;" class='location text-bold' ng-bind="ticket.priority"></span>
						</p>
					</li>
					<li ng-repeat="event in events" class="{{event.status}}">
						...
						<p>
							<span class='duration' ng-bind="event.start"></span>
							<span class='location' ng-bind="event.staff"></span>
						</p>
					</li>
					<li ng-repeat="reminder in reminders" class="next reminder"><label class="detail"><i class="ion-ios-bell"></i></label>
						<a ng-click='ReminderRead($index)' class="mark-read-reminder ion-checkmark-round" style="cursor: pointer"></a>
						<h3 class="text-bold" style="margin-bottom: 5px" ng-bind="reminder.title"><small data-toggle="popover" data-trigger="hover" title="<?php echo lang('information')?>" data-content="This reminder created by {{reminder.creator}}" data-placement="top"><i style="font-size:14px;" class="icon ion-help-circled text-muted"></i></small></h3>
						<span class="reminder-detail" style="display: table-cell;" ng-bind="reminder.description"></span>
						<p style="display: table-footer-group;"><span style="color:black;" class='duration text-bold'><span ng-bind="reminder.date"></span></span>
						</p>
					</li>
				</ul>
			</div>
