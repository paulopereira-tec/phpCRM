...
class Trivia_Model extends CI_Model {
	...
	function get_todos() {
		return $this->db->get_where( 'todo', array( 'done' => 0, 'staff_id' => $this->session->userdata( 'logged_in_staff_id' ) ) )->result_array();
	}

	function get_done_todos() {
		return $this->db->get_where( 'todo', array( 'done' => 1, 'staff_id' => $this->session->userdata( 'logged_in_staff_id' ) ) )->result_array();
	}
	...
	function get_reminders() {
		...
		$this->db->where( 'CURDATE() >= date AND date != "0000.00.00" AND isnotified != "1" AND staff_id = '. $this->session->userdata( 'logged_in_staff_id' ) .'' );
