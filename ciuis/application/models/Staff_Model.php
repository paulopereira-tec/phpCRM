...
class Staff_model extends CI_Model {
	...
	function staffsalesgraph() {
		$this->db->select_sum( 'total' );
		$this->db->from( 'sales' );
		return $this->db->get_where( '', array( 'staff_id' => $this->session->userdata( 'logged_in_staff_id' ) ) )->row()->total;
	}

	function staffmember_customer($id) {
		$this->db->from( 'customers' );
		return $this->db->get_where( '', array( 'staff_id' => $id ) )->num_rows();
	}
	function staffmember_tickets($id) {
		$this->db->from( 'tickets' );
		return $this->db->get_where( '', array( 'staff_id' => $id ) )->num_rows();
	}

	function staffmembersales( $id ) {
		$totalsales = array();
		$i = 0;
		for ( $mo = 1; $mo <= 12; $mo++ ) {
