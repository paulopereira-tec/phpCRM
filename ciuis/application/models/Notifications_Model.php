...
class Notifications_Model extends CI_Model {
	...
	function get_all_notifications() {
		...
		$this->db->where( 'public = "1" OR staff_id = ' . $this->session->userdata( 'logged_in_staff_id' ) . '' );
		$query = $this->db->get();
	...
	function readnotification() {
		$new = $this->db->get_where( 'notifications', array( 'markread' => 1,'staff_id' => $this->session->userdata('logged_in_staff_id')))->result();
		if ( $new ) {
			return '-unread';
		}
	}
	...
	function newnotification() {
		$new = $this->db->get_where( 'notifications', array( 'markread' => 0,'staff_id' => $this->session->userdata('logged_in_staff_id')))->result();
		if ( $new) {
			return 'indicator';
		}
	}
