...
class Report_Model extends CI_Model {
	...
	function newreminder() {
		$this->db->from( 'reminders' )->where( 'date <= CURDATE() AND staff_id = "' . $this->session->userdata( 'logged_in_staff_id' ) . '" AND isnotified != 1' );
		$query = $this->db->get();
		$tef = $query->num_rows();
		return $tef;
	}
	...
	function get_account_amount( $id ) {
		...
		$this->db->where( '(account_id = ' . $id . ' and transactiontype = 0)' );
		$data = $this->db->get()->row()->amount;
		if ( $data == 0 ) {
			$amount = '0';
		} else {
			$amount = $data;
	...
	function clc() {
		$this->db->from( 'leads' );
		$this->db->where( 'dateconverted != "NULL" AND assigned_id = ' . $this->session->userdata( 'logged_in_staff_id' ) . '' );
		$query = $this->db->get();
		$clc = $query->num_rows();
		return $clc;
	}
	...
	function mlc() {
		$this->db->from( 'leads' );
		$this->db->where( 'assigned_id = ' . $this->session->userdata( 'logged_in_staff_id' ) . '' );
		$query = $this->db->get();
		$mlc = $query->num_rows();
		return $mlc;
	}
	...
	function mct() {
		$this->db->from( 'tickets' );
		$this->db->where( 'status_id = 4 AND staff_id = ' . $this->session->userdata( 'logged_in_staff_id' ) . '' );
		$query = $this->db->get();
		$clc = $query->num_rows();
		return $clc;
	}
	...
	function mtt() {
		$this->db->from( 'tickets' );
		$this->db->where( 'staff_id = ' . $this->session->userdata( 'logged_in_staff_id' ) . '' );
		$query = $this->db->get();
		$mlc = $query->num_rows();
		return $mlc;
	}
	...
	function ues() {
		...
		$this->db->where( '(staff_id = ' . $this->session->userdata( 'logged_in_staff_id' ) . ' OR public = 1)' );
	...
	function tbs() { // New notification counts
		return $this->db->get_where( 'notifications', array( 'markread' => 0, 'staff_id' => $this->session->userdata( 'logged_in_staff_id' ) ) )->num_rows();;
	}
	...
	function leads_to_win_by_leadsource() {
		...
		foreach ( $statuses as $status ) {
			...
			array_push( $graphic[ 'labels' ], $status[ 'name' ] );
			array_push( $_data[ 'backgroundColor' ], '#fff' );
			array_push( $_data[ 'hoverBackgroundColor' ], ciuis_set_color( '#fff', -20 ) );
