...
class Staff extends CIUIS_Controller {
	...
	function staffmember( $id ) {
		...
		$data[ 'settings' ] = $this->Settings_Model->get_settings_ciuis();
		$data[ 'invoices' ] = $this->db->get_where( 'invoices', array( 'staff_id' => $id ) )->result_array();
		$data[ 'tickets' ] = $this->db->get_where( 'tickets', array( 'staff_id' => $id ) )->result_array();
		$this->load->view( 'staff/staffmember', $data );
