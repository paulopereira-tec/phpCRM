...
class Area extends CIUISCUSTOMER_Controller {
	...
	function markread() {
		$this->Area_Model->markread();
	}

	function invoices() {

		$data[ 'title' ] = lang( 'areatitleinvoices' );
		$data[ 'invoices' ] = $this->db->select( '*,staff.staffname as staffmembername,staff.staffavatar as staffavatar,customers.company as customer,customers.namesurname as birey,customers.address as customeraddress,invoicestatus.name as statusname, invoices.id as id ' )->join( 'customers', 'invoices.customer_id = customers.id', 'left' )->join( 'invoicestatus', 'invoices.status_id = invoicestatus.id', 'left' )->join( 'staff', 'invoices.staff_id = staff.id', 'left' )->get_where( 'invoices', array( 'customer_id' => $_SESSION[ 'customer' ] ) )->result_array();
		//Detaylar
		$data[ 'settings' ] = $this->Settings_Model->get_settings_ciuis();
	...
	function invoice( $id ) {
		...
		$this->load->view( 'area/inc/footer' );
	}
	...
	function addreply() {
		if ( isset( $_POST ) && count( $_POST ) > 0 ) {
			...
			$staffinfo = $this->Staff_Model->getUserInfo( $this->input->post( 'staff' ) );
			$setconfig = $this->Settings_Model->get_settings_ciuis();
			$this->load->library( 'email' );
			$config = array();
			$config[ 'protocol' ] = 'smtp';
			...
			$data = array(
				'name' => $_SESSION[ 'name' ],
				'ticketlink' => '' . base_url( 'tickets/ticket/' . $this->input->post( 'ticket' ) . '' ) . ''
			);
			...
			redirect( 'area/ticket/' . $this->input->post( 'ticket' ) . '' );
		}
	}
