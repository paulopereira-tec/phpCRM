function Invoice_Create(settings) {
	...
	var init = function () {
		...
		if (settings.edit) {
			init_autocomplate();
			$('.filter-money').autoNumeric({
				aSep: ASEP,
				aDec: ADEC,
				aPad: 2
			});
			$('.currency-item-label,.currency-item-edit').autoNumeric({
				aSep: ASEP,
				aDec: ADEC,
				aPad: 4
			});
			$('.filter-number').numeric();
			if ((settings.payment_count == 0) || settings.copy) $('input[name=status]').change();
		} else {
			$('#add-line').click();
			$('input[name=status]').change();
		}
		$('.payment-currency-label,.payment-currency-edit').autoNumeric({
			aSep: ASEP,
			aDec: ADEC,
			aPad: 4,
			mDec: 4
		});
		$('.payment-total-label,.payment-total-edit').autoNumeric({
			aSep: ASEP,
			aDec: ADEC,
			aPad: 2
		});
	...
	$(document).on('keyup', '.currency-item-edit,.payment-currency-edit', function (a) {
		if (a.keyCode == 13) $(this).blur();
	});
